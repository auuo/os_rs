...
use os_rs::{exit_qemu, QemuExitCode, serial_print};
    ...
    static ref TEST_IDT: InterruptDescriptorTable = {
        ...
        unsafe {
            idt.double_fault.set_handler_fn(test_double_fault_handler)
                .set_stack_index(os_rs::gdt::DOUBLE_FAULT_IST_INDEX);
        }
        idt
    };
}
...
fn stack_overflow() {
    stack_overflow(); // 递归触发栈溢出
    volatile::Volatile::new(0).read(); // 避免尾递归优化
}
